...
int igualAreaDescarte(Lista *areaDescarte, Pilha *pilhaJogador, Carta cartaDaVez) {
  if (buscarItem(areaDescarte, cartaDaVez.numero) != -1) {
    ElementoLista *elementoRemovido = removerItem(areaDescarte, buscarItem(areaDescarte, cartaDaVez.numero));
    Carta cartaRemovida = elementoRemovido->carta;
    free(elementoRemovido);

    empilhar(pilhaJogador, cartaRemovida);
    empilhar(pilhaJogador, cartaDaVez);

    printf("Sua carta da vez é igual a uma carta da Área de Descarte. Você recebeu 2 cartas!\n");
    return 0;
  } else {
    printf("Sua carta da vez não é igual a nenhuma carta da Área de Descarte.\n");
    return 1;
  }
}
...
int main (){
  ...
  Lista *areaDescarte = criarLista();
  Pilha *pilhaJogador = criarPilha();
  ...
  inserirItem(areaDescarte, carta1);
  inserirItem(areaDescarte, carta2);
  inserirItem(areaDescarte, carta3);

  igualAreaDescarte(areaDescarte, pilhaJogador, cartaDaVez);
