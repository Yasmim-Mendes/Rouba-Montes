...
int igualMonteAdversario(Pilha *pilhaJogador, Carta cartaDaVez, int njogadores, Jogador jogadorAtual, Jogador jogador) {
  Pilha pilhaTemporaria; // Pilha temporária para evitar loops infinitos
  pilhaTemporaria.topo = NULL;

  while (pilhaJogador->topo != NULL) {
    if (cartaDaVez.numero == pilhaJogador->topo->carta.numero) {
      while (pilhaJogador->topo != NULL) {
        empilhar(&pilhaTemporaria, desempilhar(pilhaJogador));
      }

      printf("Sua carta da vez é igual a carta do topo do monte do %s. Você recebeu todas as cartas do monte do %s!\n", jogador.nome, jogador.nome);

      while (pilhaTemporaria.topo != NULL) {
        empilhar(pilhaJogador, desempilhar(&pilhaTemporaria));
      }
  ...
  printf("Sua carta da vez não é igual a nenhuma carta do topo do monte de seus adversários.\n");
  while (pilhaTemporaria.topo != NULL) {
    empilhar(pilhaJogador, desempilhar(&pilhaTemporaria));
  }
...
int main() {
    // Inicializando o monte do jogador
    Pilha monteJogador;
    monteJogador.topo = NULL;

    // Definindo a carta da vez do jogador
    Carta cartaDaVez;
    cartaDaVez.numero = 4;
    cartaDaVez.naipe = 'C';

    // Inicializando informações do jogador
    Jogador jogadorAtual;
    jogadorAtual.id = 0;
    strcpy(jogadorAtual.nome, "Jogador 1");

    Jogador jogadorAdversario;
    jogadorAdversario.id = 1;
    strcpy(jogadorAdversario.nome, "Jogador 2");

    // Criando um monte do jogador adversário com uma carta igual à carta da vez
    ElementoPilha *cartaTopoAdversario = (ElementoPilha *)malloc(sizeof(ElementoPilha));
    cartaTopoAdversario->carta.numero = 4;
    cartaTopoAdversario->carta.naipe = 'C';
    cartaTopoAdversario->anterior = NULL;
    monteJogador.topo = cartaTopoAdversario;

    // Chamando a função igualMonteAdversario
    int resultado = igualMonteAdversario(&monteJogador, cartaDaVez, 2, jogadorAtual, jogadorAdversario);

    // Liberando a memória alocada para o monte do jogador
    free(cartaTopoAdversario);
